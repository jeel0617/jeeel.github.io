document.addEventListener('DOMContentLoaded', function () {
    ...
    checkout.addEventListener("click", function () {
        ...
        cartItems.forEach(item => {
            receiptWindow.document.write(`<div>${item.name} x${item.qty} - ₱${(item.qty * item.price).toFixed(2)}</div>`);
        });
    
        receiptWindow.document.write(`
            <div style="margin-top: 20px; font-weight: bold;">Total: ₱${totalAmount.toFixed(2)}</div>
